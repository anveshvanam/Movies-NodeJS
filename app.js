...
app.get("/movies/:movieId/", async (request, response) => {
  ...
  response.send(ConvertMovieDb(getMovieDetailsQueryResponse));
});
...
const convertDirectorDb = (objectItem) => {
  return {
    directorId: objectItem.director_id,
...
app.get("/directors/", async (request, response) => {
  ...
  response.send(
    getDirectorsQueryResponse.map((eachItem) => convertDirectorDb(eachItem))
  );
});
